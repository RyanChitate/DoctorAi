...
GREETINGS_OUT = ["Hi, I'm DrAi", "Aiiii i'm DrAi"]
...
FAREWELLS_OUT = ["Say bye to DrAi", "Bye bye, I've been DrAi"]
...
def preprocess_text(text):
    ...
    # Remove stopwords and single-character tokens, excluding specific words
    stop_words = set(stopwords.words('english'))
    tokens = [word for word in tokens if word.isalnum() and word not in stop_words and word.lower() not in ['hello', 'hi', 'hey', 'hola', 'welcome', 'good', 'morning', 'afternoon', 'evening', 'thank', 'thanks', 'appreciate', 'lot', 'bye', 'goodbye', 'later', 'ya', 'take', 'care']]
    return ' '.join(tokens)
...
def get_bot_response(user_input, conversation_history):
    ...
    if "hi drai" in user_input.lower():
...
def main():
    ...
    if user_input:
        ...
        st.markdown('<div class="chat-container"><div class="bot-bubble">' + bot_response +  '</div></div>', unsafe_allow_html=True)
